...
export function NewyearYearClient({ data, year }: NewyearYearClientProps) {
  ...
  // 関連リンクを生成
  const relatedLinks = generateYearDetailLinks('newyear', year.toString())
  ...
  const handleSwipeLeft = () => {
    ...
    if (nextYear) router.push(`/ekiden/newyear/${nextYear}`)
  }

  const handleSwipeRight = () => {
    const prevYear = getPrevYear()
    if (prevYear) router.push(`/ekiden/newyear/${prevYear}`)
  }
