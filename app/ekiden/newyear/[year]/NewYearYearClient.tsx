...
import { getMedalEmoji, normalizeForSearch, removeLeadingZero, shortenUniversityName } from "@/lib/format-utils"
      ...
      <div className="bg-white border-b">
        <div className="container mx-auto px-4 lg:px-8 py-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-1">
            ニューイヤー駅伝{year}結果速報 | 区間記録・{data.teams?.[0]?.name ? `優勝${data.teams[0].name}・` : ''}成績一覧
          </h1>
        </div>
      </div>
                        ...
                        <table className="min-w-full bg-white section-results-table">
                          <thead>
                            <tr className="bg-gray-100 text-gray-600 text-sm leading-normal">
                              ...
                              <th className="py-3 px-2 md:px-4 text-left">チーム</th>
                              ...
                              <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                                <td className="py-2 px-2 md:px-4 md:whitespace-nowrap">
                                  <div className="flex flex-col md:block gap-1">
                                    ...
                                    <span className="text-xs md:hidden text-gray-700 break-words">{runner.name}</span>
                                  </div>
                                </td>
                                <td className="py-2 px-2 md:px-4 whitespace-nowrap hidden md:table-cell">{runner.name}</td>
