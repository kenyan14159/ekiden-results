...
import { fujisanResults } from "@/data/fujisan-results"
...
export default function FujisanEkidenPage() {
  ...
  const relatedLinks = generateRaceListLinks('fujisan')

  // クライアント側でyearsを生成
  useEffect(() => {
    setYears(fujisanResults.map(result => result.year))
  }, [])


  const filteredResults = selectedDecade
    ? fujisanResults.filter(result => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      {years.length > 0 && (
        <RaceListStructuredDataScript raceSlug="fujisan" years={years} />
      )}
      <Header />
        ...
        <div className="container mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
          <div className="space-y-2 sm:space-y-3">
            {filteredResults.map((result) => (
              <Link key={result.year} href={`/ekiden/fujisan/${result.year}`}>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 sm:gap-4 min-w-fit">
                      ...
                      {result.fujiCount && (
                        <div className="text-center min-w-[50px] sm:min-w-[60px]">
                          <p className="text-xs text-gray-500">富士山</p>
                          <p className="text-lg font-bold text-blue-600">{result.fujiCount}</p>
                          <p className="text-xs text-gray-500">回</p>
                        </div>
                      )}
                      <div className="text-center min-w-[50px] sm:min-w-[60px]">
